...
def add(A, B):
    ...
    for r in B.f:
        if (r in A.f):
            new_mat[r] = A.f[r] + B.f[r]
        else:
            new_mat.update({r:B.f[r]})
    return Mat(A.D, new_mat)
...
def scalar_mul(M, x):
    ...
    return Mat(M.D, {r:(M.f[r]*x) for r in M.f})
...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    possible = {(x,y) for x in A.D[0] for y in A.D[1]}
    for r in possible:
        if (r in A.f):
            if (r not in B.f):
                if (A.f[r] != 0):
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1],M.D[0]), {(r,p):M.f[(p,r)] for (p,r) in M.f})
...
def vector_matrix_mul(v, M):
    ...
    for (c,r) in M.f:
        if c in v.f:
            if r in to_return:
                to_return[r] = to_return[r] + (v.f[c])*M.f[(c,r)]
            else:
                to_return[r] = (v.f[c])*M.f[(c,r)]
    return Vec(M.D[1].copy(),to_return)
...
def matrix_vector_mul(M, v):
    ...
    for (c,r) in M.f:
        if r in v.f:
            if c in to_return:
                to_return[c] = to_return[c] + M.f[(c,r)]*(v.f[r])
            else:
                to_return[c] = M.f[(c,r)]*(v.f[r])
    return Vec(M.D[0].copy(), to_return)
...
def matrix_matrix_mul(A, B):
    ...
    for (r,c) in A.f:
        for m in B.D[1]:
            if (c,m) in B.f:
                if (r,m) in to_return:
                    to_return[(r,m)] = to_return[(r,m)] + A.f[(r,c)]*B.f[(c,m)]
                else:
                    to_return[(r,m)] = A.f[(r,c)]*B.f[(c,m)]
    return Mat((A.D[0].copy(),B.D[1].copy()),to_return)
